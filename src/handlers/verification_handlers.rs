...
pub async fn send_otp_to_email(
    ...
    // email body
    let email =
        "OTP for your email verification is ".to_string() +
        &otp +
...
pub async fn verify_otp(
    ...
    let result: Option<OTP> = db
        .select(("otp", otp_verification_request.email.clone())).await
        .unwrap();
                ...
                Utc.from_local_datetime(&chrono::Local::now().naive_local()).single().unwrap()
            {
                ...
                if otp.otp == otp_verification_request.otp.clone() {
                    ...
                    let _response: Option<OTP> = db
                        .delete(("otp", otp_verification_request.email.clone())).await
                        .unwrap();
                ...
                let _response: Option<OTP> = db
                    .delete(("otp", otp_verification_request.email)).await
                    .unwrap();
                Err(StatusCode::BAD_REQUEST)
            }
        }
