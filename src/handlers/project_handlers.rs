...
pub async fn create_a_project(
    ...
    match project_request.content {
        None => {
            return (StatusCode::BAD_REQUEST, Json(ProjectCreateResponse {
    ...
    match new_project.save(db, Some(claim.get_surrealdb_thing())).await {
        Ok(_) => {
            return (StatusCode::OK, Json(ProjectCreateResponse {
                message: "Project created successfully".to_string()
            }))
        },
        Err(e) => {
            println!("Error: {}", e);
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(ProjectCreateResponse {
