...
impl Undergraduate {
            ...
            output: Some(
                Output::Fields(
                    Fields(
                        vec![
                            Field::Alone(
                                Value::Idiom(Idiom(vec![Part::Field(Ident("id".to_string()))]))
                            )
                        ],
                        true
                    )
            ...
            data: Some(
                Data::SetExpression(
                    vec![(
                        Idiom(
                            vec![
                                Part::Field(Ident("university_email_verification_flag".to_string()))
                            ]
                        ),
                        Operator::Equal,
                        Value::True,
            ...
            cond: Some(
                Cond(
                    Value::Expression(
                        Box::from(Expression {
                            l: Value::Idiom(
                                Idiom(vec![Part::Field(Ident("university_email".to_string()))])
                            ),
    ...
    pub async fn update_university_details(
        ...
        let response: Result<String, surrealdb::Error> = db
            ...
            .patch(PatchOp::replace("/university_email_verification_flag", false)).await;
