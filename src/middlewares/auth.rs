...
pub async fn validate_jwt<T>(
    ...
    if request.headers().contains_key("Authorization") {
        ...
        let authorization_header = request.headers().get("Authorization").unwrap().to_str().unwrap().to_string();
    
        let token = jwt::verify_jwt(
            authorization_header.split(" ").collect::<Vec<&str>>()[1].to_string()
        ).await ?;
